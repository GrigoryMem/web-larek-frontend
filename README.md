...
События формы
`имя формы.имя поля ввода:change` - уведомляем об изменении поля ввода и подписываемся на него через регулярное выражение:???
`имя формы.имя поля ввода:submit` - уведомляем,что пользователь нажал кнопку подтвержения отправки данных формыи всего заказа, конкретно через регулярные выражения:????
